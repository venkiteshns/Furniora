...
import { markItemAsSold } from "../store/userProductSlice";
  ...
  async function handleRemove(id) {
    try {
      ...
      if (status?.acknowledged && status?.deletedCount === 1) {
        ...
        return showSuccess("Product removed from cart");
      }
  ...
  const checkOut = async () => {
    ...
    try {
      ...
      if (response === "failed") {
        ...
        dispatch(markItemAsSold(product));
  ...
  return products?.length > 0 ? (
    <div className="max-w-4xl mx-auto p-6 md:p-8 bg-white shadow-xl rounded-xl mt-10 mb-10">
      ...
      <div className="flex flex-col space-y-6">
        {products?.map((item) => (
          <div
            key={item._id}
            className="relative flex flex-col sm:flex-row items-center justify-between p-5 border border-gray-100 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 bg-gray-50"
          >
            {/* Delete Icon */}
            <button
              onClick={async () => {
    ...
    <div className="flex flex-col items-center justify-center py-16 px-4 md:px-8 max-w-4xl mx-auto bg-white shadow-xl rounded-xl mt-10 mb-10">
      ...
      <p className="text-gray-500 text-lg mb-8 max-w-md text-center">
        It looks like you haven't added any products to your cart yet. Browse
        our catalog to find items that suit your needs.
      </p>
      <NavLink to={"/"}>
        <button
          type="button"
